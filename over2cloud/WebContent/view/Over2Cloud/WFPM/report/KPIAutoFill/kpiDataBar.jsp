...
function showDataValue()
{
	...
	$.ajax({
		type : "post",
		url  : "view/Over2Cloud/WFPM/report/DSR/beforeKPIAutofill.action",
...
function initializeGraph()
{
	...
	$.ajax({
	    type : "post",
	    url : "view/Over2Cloud/WFPM/report/DSR/kpiAutofillGraph.action?currentMonthValue="+dateValue,
	    type : "json",
	    success : function(data) {
		kpiData = data;
	    	var settings = {
	
	    			title: 'KPI Target Vs Achievement',
	                description: '',
...
function getgraph4ParticularKPI(kpi,date)
{
	dateValue=$("#"+date).val();
	var kpiId=$("#"+kpi).val();
	if (kpiId=='-1') 
	...
	else 
	{
		...
		$.ajax({
		    type : "post",
		    url : "view/Over2Cloud/WFPM/report/DSR/kpiAutofillGraphPerKpi.action?kpiId="+kpiId+"&currentMonthValue="+dateValue,
		    type : "json",
		    success : function(data) {
			kpiData1 = data;
		          // prepare jqxChart settings
		          var settings = {
	        		  title: 'KPI Target vs Achievement (Per Day)',
		              description: "",
		              enableAnimations: true,
		              showLegend: true,
...
function fetchAchievedKPIForMonth(date) //date=dd:mm:yyyy
{
	$.ajax({
		type : "post",
		url  : "view/Over2Cloud/WFPM/report/DSR/fetchAchievedKPIForMonth.action",
		data: "currentMonthValue="+date,
		success : function(data){
			$('#kpiSelect option').remove();
			$('#kpiSelect').append($("<option></option>").attr("value",-1).text('All KPI'));
			$(data.jsonArrayKPI).each(function(index)
			{
			   $('#kpiSelect').append($("<option></option>").attr("value",this.ID).text(this.NAME));
			});
		},
		error   : function(){
			alert("error");
...
<div class="listviewBorder" style="margin: 10px; width: 98%;" align="center">
	...
	<div class="pwie"><table class="lvblayerTable secContentbb" border="0" cellpadding="0" cellspacing="0" width="100%"><tbody><tr></tr><tr><td></td></tr><tr><td> <table class="floatL" border="0" cellpadding="0" cellspacing="0"><tbody><tr><td class="pL10"> 
		<sj:datepicker cssStyle="height: 16px; width: 79px;" onchange="fetchAchievedKPIForMonth(this.value); getgraph4ParticularKPI('kpiSelect','fromDate');" cssClass="button" 
			id="fromDate" name="fromDate" size="20" maxDate="0" value="today" readonly="true"   yearRange="2013:2050" showOn="focus" 
		...
		<s:select  
					...
					cssClass			=		"button"
               		...
               		onchange			=		"getgraph4ParticularKPI('kpiSelect','fromDate');"
                         	>
        </s:select>
