...
<div style="overflow:auto; width: 96%; margin: 1%; border: 1px solid #e4e4e5; -webkit-border-radius: 6px; -moz-border-radius: 6px; border-radius: 6px; -webkit-box-shadow: 0 1px 4px rgba(0, 0, 0, .10); -moz-box-shadow: 0 1px 4px rgba(0, 0, 0, .10); box-shadow: 0 1px 4px rgba(0, 0, 0, .10); padding: 1%;">
    			<s:form id="formone" name="formone" namespace="/view/Over2Cloud/AssetOver2Cloud/NetworkMonitor" action="AddIpPort" theme="simple"  method="post" enctype="multipart/form-data" >
				<center>
	   ...
	   <div id="assetDetailsDiv">
				...
				<s:iterator value="assetColumnMap" begin="0" end="0" status="status">
                    ...
                    <div class="newColumn">
                            ...
                            <s:if test="%{mandatory}"><span class="needed"></span></s:if>
                  		    <s:textfield  id="assetname" maxlength="50"  placeholder="Enter Data" readonly="true"  cssClass="textField"/>
                            </div>
	                  </div>
                </s:iterator>
                
                <s:iterator value="assetColumnMap" begin="1" status="status">
                      <s:if test="%{mandatory}">
               		       <span id="normal" class="pIds" style="display: none; "><s:property value="%{key}"/>#<s:property value="%{value}"/>#<s:property value="%{colType}"/>#<s:property value="%{validationType}"/>,</span>
                      </s:if>
				 	<s:if test="#status.odd">
					  <div class="newColumn">
							...
							<div class="rightColumn">
                            <s:if test="%{mandatory}"><span class="needed"></span></s:if>
                  		    <s:textfield  id="%{Key}11" maxlength="50"  placeholder="Enter Data" readonly="true"  cssClass="textField"/>
                            </div>
	                  </div>
					</s:if>
             		<s:else>
				...
				<s:iterator value="netMonitorColumnMap" status="status">
                      <s:if test="%{mandatory}">
               		       <span id="normal" class="pIds" style="display: none; "><s:property value="%{key}"/>#<s:property value="%{value}"/>#<s:property value="%{colType}"/>#<s:property value="%{validationType}"/>,</span>
                      </s:if>
				 	<s:if test="#status.odd">
					  <div class="newColumn">
							...
							<div class="rightColumn">
                            <!--<s:if test="%{mandatory}"><span class="needed"></span></s:if>
                  		    --><s:textfield  id="%{key}" name="%{key}" maxlength="50"  placeholder="Enter Data"   cssClass="textField"/>
                            </div>
	                  </div>
					</s:if>
             		<s:else>
				...
				<s:iterator value="commonDropDownList">
	                 <s:if test="%{mandatory}">
	                    <span id="mandatoryFields" class="pIds" style="display: none; "><s:property value="%{key}"/>#<s:property value="%{value}"/>#<s:property value="%{colType}"/>#<s:property value="%{validationType}"/>,</span>
                          ...
                          <s:select  
	                          id="%{key}"
	                          list="#{'IP':'IP Scan','Port':'Port Scan'}"
	                          name="%{key}"
	                          cssClass="select"
	                          cssStyle="width:82%"
                          >
