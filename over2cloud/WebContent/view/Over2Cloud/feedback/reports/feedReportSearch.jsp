...
function viewSearchdData()
{
    ...
    $.ajax({
        type : "post",
        url : conP+"/view/Over2Cloud/feedback/report/beforeSearchedDataViewFeedback.action?startDate="+startDate+"&endDate="+endDate+"&patType="+patType+"&mode="+mode.split(" ").join("%20"),
			...
			<div class="newColumn">
				...
				<div class="rightColumn">
					<s:select 
                      id="mode" 
			          name="mode" 
			          list="{'Paper','SMS','Ward Rounds','Administrative Rounds'}"
			          headerKey="-1"
			          headerValue="Select Mode"
			          cssClass="select"
			          cssStyle="width:82%"
         			 />
				</div>
