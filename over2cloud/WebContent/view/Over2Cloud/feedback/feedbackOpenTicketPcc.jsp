...
<body>
		...
		<div id="errZone11" style="float:center; border-color: black; background-color: rgb(255,99,71); color: white;width: 100%; font-size: small; border: 0px solid red; border-radius: 6px;" align="center"></div>
		
		<s:form id="formone" name="formone" action="feedbackViaOnlineTicketPcc"  theme="css_xhtml"  method="post" enctype="multipart/form-data" namespace="/view/Over2Cloud/feedback">
		<s:hidden name="viaFrom" value="online"></s:hidden>
		<s:hidden name="tosubdept" id="tosdId"></s:hidden>
		<s:hidden value="%{clientId}" name="clientId"></s:hidden>
		<s:hidden value="%{compType}" name="compType"></s:hidden>
		<s:hidden value="%{visitType}" name="visitType"></s:hidden>
		<s:hidden value="%{clientName}" name="clientName"></s:hidden>
		<s:hidden value="%{mobNo}" name="mobileno"></s:hidden>
		<s:hidden value="%{emailID}" name="emailId"></s:hidden>
		<s:hidden value="%{comments}" name="comments"></s:hidden>
          	...
          	<s:iterator value="feedbackTextColumns" status="status">
          	 <span id="mandatoryFields" class="pIds" style="display: none; "><s:property value="%{key}"/>#Complaint Details#D#,</span>
                 ...
                 <div class="newColumn">
                       ...
                       <div class="rightColumn1"><span class="needed"></span><s:textarea name="%{key}" id="%{key}"  placeholder="Enter Data" cssStyle="margin:0px 0px 10px 0px"  cssClass="textField" /><s:if test="key=='feed_brief'"><div id="errorfeedbrief" class="errordiv"/></s:if></div>
          	    </div>	
          	     </s:if>
          	     <s:else>
          	     <div class="newColumn">
				...
				<li class="submit" style="background: none;">
	                ...
	                <center><sj:submit 
						...
						<sj:submit 
		                     value="Reset" 
		                     button="true"
		                     cssStyle="margin-left: 152px;margin-top: -40px;"
