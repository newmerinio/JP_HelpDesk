...
<div class="page_form">
  ...
  <s:form id="cmo" name="cmo" action="cmoAddAction" namespace="/view/Over2Cloud/ProductivityEvaluationOver2Cloud" theme="simple"  method="post" enctype="multipart/form-data" >
    <center>
	...
	<s:hidden id="moduleName" name="moduleName" value="%{moduleName}"/>
          ...
          <s:if test="%{field_value=='agenda'}">
                ...
                <s:select  
                             id		    ="%{field_value}"
                             name		="%{field_value}"
                             list		="#{'OG Review':'OG Review','Project Review':'Project Review','Budget Review':'Budget Review' }"
                             headerValue="Select Agenda"
                             headerKey	="-1"
                             cssClass="select"
							 cssStyle="width:82%"
                             >
		  ...
		  <s:iterator value="productivityTxtBox" status="status"  begin="0" end="1">
		    ...
		    <s:iterator value="productivityDate" status="status" >
			 <s:if test="%{field_value=='forMonth'}">
			             <div class="newColumn">
			               <div class="leftColumn"><s:property value="%{field_name}"/>:</div>
                		   <div class="rightColumn">
				              <s:if test="%{mandatory}"><span class="needed"></span></s:if>
			 ...
			 <s:elseif test="%{field_value=='onDate'}">
			  <div class="newColumn">
			               <div class="leftColumn"><s:property value="%{field_name}"/>:</div>
                		   <div class="rightColumn">
				              <s:if test="%{mandatory}"><span class="needed"></span></s:if>
		  ...
		  <s:iterator value="productivityTime" status="status" >
		    ...
		    <s:iterator value="productivityTxtBox" status="status" begin="2">
			 <s:if test="#status.odd==true">
			             <div class="newColumn">
  	            		 <div class="leftColumn"><s:property value="%{field_name}"/>:</div>
                		 <div class="rightColumn">
          ...
          <s:if test="%{field_value=='frequency'}">
                ...
                <s:select  
                             ...
                             cssClass="select"
							 cssStyle="width:82%;"
							 onchange="getDueDatePicker(this.value,'dueDateDiv')"
                             >
                   </s:select>
                </div>
                </div>
           ...
           <s:iterator value="productivityDate" status="status" >
			 <s:if test="%{field_value=='dueDate'}">
			             <div class="newColumn">
			               <div class="leftColumn"><s:property value="%{field_name}"/>:</div>
                		   <div class="rightColumn">
				              <s:if test="%{mandatory}"><span class="needed"></span></s:if>
				              		<sj:datepicker id="%{field_value}"  name="%{field_value}" value="today" readonly="true" cssClass="textField" size="20"   changeMonth="true" changeYear="true"  yearRange="2013:2050" showOn="focus" displayFormat="dd-mm-yy" Placeholder="Select Date"/>
				         </div>
			             </div>
			 </s:if>
		  </s:iterator>
