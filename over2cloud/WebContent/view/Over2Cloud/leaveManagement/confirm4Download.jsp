...
<s:form action="downloadAction" id="download" theme="css_xhtml">
	...
	<s:if test="download4 == 'dailyReport'">
	<s:hidden name="empname" value="%{empname}" />
	<s:hidden name="fdate" value="%{fdate}" />
	<s:hidden name="tdate" value="%{tdate}" />
	</s:if>
	<s:if test="download4 == 'summaryReport'">
	<s:hidden name="empname" value="%{empname}" />
	</s:if>
	 <br>
	 <s:iterator value="columnMap" status="rowstatus">
                   ...
                   <s:property value="%{value}"/>
                    <sj:datepicker name="%{key}" id="%{key}" value="" changeMonth="true" changeYear="true" yearRange="2013:2020" showOn="focus" displayFormat="dd-mm-yy" cssStyle="margin:0px 0px 10px 0px; margin-left: 61px;margin-top: -18px; width: 204px;"  cssClass="textField" placeholder="Select From date"/>
                    </s:if>
				   <s:if test="%{key=='tdate'}">
                   <s:property value="%{value}"/>
                    <sj:datepicker name="%{key}" id="%{key}" value="" changeMonth="true" changeYear="true" yearRange="2013:2020" showOn="focus" displayFormat="dd-mm-yy" cssStyle="margin:0px 0px 10px 0px; margin-left: 61px;margin-top: -18px; width: 204px;"  cssClass="textField" placeholder="Select To date"/>
                 </s:if>
    </s:iterator>
     <br>
		<sj:div id="sortable" sortable="true" sortableOpacity="0.1" sortablePlaceholder="ui-state-highlight" sortableForcePlaceholderSize="true" cssStyle="width: 200px;">
