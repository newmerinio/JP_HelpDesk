...
function addTerritory(){
	...
	$.ajax({
	    type : "post",
	    url : "view/Over2Cloud/patientCRMMaster/beforeAddTerritory.action",
...
function getGridView()
{
	$.ajax({
 		type :"post",
 		url :"view/Over2Cloud/patientCRMMaster/viewTerritoryDetails.action",
...
<div class="list-icon">
 ...
 <div class="clear"></div>
	...
	<sj:a id="addButton1"  button="true" title="Add Territroy"  buttonIcon="ui-icon-plus" cssClass="button" onclick="addTerritory();">Add Territory</sj:a>
   <sj:a id="addButton2"  button="true" title="Back Location View"  buttonIcon="ui-icon-plus" cssClass="button" onclick="viewLocation();">Location</sj:a>
...
var selRowIds = jQuery('#gridTerritory').jqGrid('rowselect', 'selarrrow');
...
function editRow()
{
	...
	{
		jQuery("#gridTerritory").jqGrid('editGridRow', row ,{height:230, width:425,reloadAfterSubmit:false,closeAfterEdit:true,top:0,left:350,modal:true , afterSubmit: function () {
			reloadPage();
	    }});
...
function deleteRow()
{
	  var status = jQuery("#gridTerritory").jqGrid('getCell',row,'status');
//	  alert(status);
	if(row==0)
	{
	...
	{
		$("#gridTerritory").jqGrid('delGridRow',row, {height:120, width:425,reloadAfterSubmit:true,closeAfterEdit:false,top:150,left:350,modal:true,caption:'Delete',msg: "Delete selected record(s)?",bSubmit: "Delete" , afterSubmit: function () {
			reloadPage();
	    }});
...
function searchRow()
{
	 $("#gridTerritory").jqGrid('searchGrid', {sopt:['eq','cn']} );
}

function reloadPage()
{
	var grid = $("#gridTerritory");
