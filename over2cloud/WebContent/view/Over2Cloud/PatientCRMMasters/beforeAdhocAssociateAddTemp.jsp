...
<script type="text/javascript">
	function closeAdd() {
		$("#addDialogTemp").dialog('close');
		backToPatientForm();
	}
</script>
<script type="text/javascript">
	function backToPatientForm() {
		 $("#addNewRefferalDialog").dialog('close');
		 $("#formReffDiv").val("");
	}
...
<body>
	...
	<div class="middle-content">
		...
		<div class="list-icon">
			<div class="head">Adhoc Referral Details</div>
			<div class="head">
		...
		<div class="border">
			<div class="container_block">
				<div style="float: left; padding: 20px 1%; width: 98%;">
					<s:form id="formAdohocAssociate" name="formone"
						namespace="/view/Over2Cloud/patientCRMMaster"
						action="addAdhocAssociateDetails" theme="css_xhtml" method="post"
				...
				<s:iterator value="corporateTextBox" status="status" >
		     	...
		     	<span id="complSpan" class="pIds" style="display: none; "><s:property value="%{key}"/>#<s:property value="%{value}"/>#<s:property value="%{colType}"/>#<s:property value="%{validationType}"/>,</span>
				    ...
				    <s:if test="%{mandatory}"><span class="needed"></span></s:if>
						...
						<s:if test="%{ key == 'asso_type'}">
						 <s:select 
                                      id="%{key}"
                                      name="%{key}" 
                                      list="%{catList}"
                                      headerKey="-1"
                                      headerValue="--Select AdhocAssociate Type--"
                                      cssClass="select"
                                      cssStyle="width:82%"
                                      theme="simple"
                                      >
						...
						<s:elseif test="%{key == 'asso_state'}">
							        <s:select 
                                      ...
                                      cssStyle="width:82%"
                                      theme="simple"
                                      onchange=" getCity(this.value,'asso_city')"
                                      >
                          </s:select>
							       </s:elseif>
							       <s:elseif test="%{key == 'asso_city'}">
							        <s:select 
                                      id="%{key}"
                                      name="%{key}" 
						...
						<div class="clear">
							<div style="padding-bottom: 10px; margin-left: -76px"
								...
								<sj:a cssStyle="margin-left: 276px;margin-top: -58px;"
									button="true" href="#" value="Back" onclick="closeDialogById('addNewRefferalDialog'); backToPatientForm();"
