...
function addNew(){
	...
	$.ajax({
	    type : "post",
	    url : "view/Over2Cloud/patientCRMMaster/beforeAddBandEmployeeMapping.action",
...
function getGridView()
{
	$.ajax({
 		type :"post",
 		url :"view/Over2Cloud/patientCRMMaster/viewBandEmployeeMappingDetails.action",
...
<body>
<div class="list-icon">
	 <div class="head">Map Employee to Band</div>
...
var selRowIds = jQuery('#gridBandEmployeeMapping').jqGrid('rowselect', 'selarrrow');
...
function editRow()
{
	...
	{
		jQuery("#gridBandEmployeeMapping").jqGrid('editGridRow', row ,{height:230, width:425,reloadAfterSubmit:false,closeAfterEdit:true,top:0,left:350,modal:true , afterSubmit: function () {
			reloadPage();
	    }});
...
function deleteRow()
{
	  var status = jQuery("#gridBandEmployeeMapping").jqGrid('getCell',row,'status');
//	  alert(status);
	if(row==0)
	{
	...
	{
		$("#gridBandEmployeeMapping").jqGrid('delGridRow',row, {height:120, width:425,reloadAfterSubmit:true,closeAfterEdit:false,top:150,left:350,modal:true,caption:'Delete',msg: "Delete selected record(s)?",bSubmit: "Delete" , afterSubmit: function () {
			reloadPage();
	    }});
...
function searchRow()
{
	 $("#gridBandEmployeeMapping").jqGrid('searchGrid', {sopt:['eq','cn']} );
}

function reloadPage()
{
	var grid = $("#gridBandEmployeeMapping");
