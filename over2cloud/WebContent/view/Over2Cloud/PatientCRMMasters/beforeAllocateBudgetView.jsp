...
function getGridView()
{
	$.ajax({
 		type :"post",
 		url :"view/Over2Cloud/patientCRMMaster/viewAllocateBudgetDetails.action",
...
<body>
	...
	<div class="list-icon">
		<div class="list-icon">
		<div class="head">
		<div id="Cid" class="head">Allocate Budget</div><div class="head">
...
var selRowIds = jQuery('#gridSourceMaster1').jqGrid('rowselect', 'selarrrow');
...
function editRow()
{
	...
	{
		jQuery("#gridSourceMaster1").jqGrid('editGridRow', row ,{height:230, width:425,reloadAfterSubmit:true,closeAfterEdit:true,top:0,left:350,modal:true , afterSubmit: function () {
			reloadPage();
			allocate_budget();
	    }});
...
function deleteRow()
{
	  var status = jQuery("#gridSourceMaster1").jqGrid('getCell',row,'status');
//	  alert(status);
	if(row==0)
	{
	...
	{
		$("#gridSourceMaster1").jqGrid('delGridRow',row, {height:120, width:425,reloadAfterSubmit:true,closeAfterEdit:false,top:150,left:350,modal:true,caption:'Delete',msg: "Delete selected record(s)?",bSubmit: "Delete" , afterSubmit: function () {
			reloadPage();
			allocate_budget();
	    }});
...
function searchRow()
{
	 $("#gridSourceMaster1").jqGrid('searchGrid', {sopt:['eq','cn']} );
}

function reloadPage()
{
	var grid = $("#gridSourceMaster1");
