...
function addNew(){
	...
	$.ajax({
	    type : "post",
	    url : "view/Over2Cloud/patientCRMMaster/beforeAddCurrencyTypeMaster.action",
...
function getGridView()
{
	$.ajax({
 		type :"post",
 		url :"view/Over2Cloud/patientCRMMaster/viewCurrencyTypeMasterDetails.action",
...
<body>
<div class="list-icon">
	 <div class="head">Currency Type</div>
...
var selRowIds = jQuery('#gridCurrencyTypeMaster').jqGrid('rowselect', 'selarrrow');
...
function editRow()
{
	...
	{
		jQuery("#gridCurrencyTypeMaster").jqGrid('editGridRow', row ,{height:230, width:425,reloadAfterSubmit:false,closeAfterEdit:true,top:0,left:350,modal:true , afterSubmit: function () {
			reloadPage();
	    }});
...
function deleteRow()
{
	  var status = jQuery("#gridCurrencyTypeMaster").jqGrid('getCell',row,'status');
//	  alert(status);
	if(row==0)
	{
	...
	{
		$("#gridCurrencyTypeMaster").jqGrid('delGridRow',row, {height:120, width:425,reloadAfterSubmit:true,closeAfterEdit:false,top:150,left:350,modal:true,caption:'Delete',msg: "Delete selected record(s)?",bSubmit: "Delete" , afterSubmit: function () {
			reloadPage();
	    }});
...
function searchRow()
{
	 $("#gridCurrencyTypeMaster").jqGrid('searchGrid', {sopt:['eq','cn']} );
}

function reloadPage()
{
	var grid = $("#gridCurrencyTypeMaster");
