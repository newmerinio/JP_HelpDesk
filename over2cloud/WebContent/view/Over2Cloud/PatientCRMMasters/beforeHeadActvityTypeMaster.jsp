...
function addNew(){
	...
	$.ajax({
	    type : "post",
	    url : "view/Over2Cloud/patientCRMMaster/beforeAddActivityTypeMaster.action",
...
function getGridView()
{
	$.ajax({
 		type :"post",
 		url :"view/Over2Cloud/patientCRMMaster/viewActivityTypeMasterDetails.action",
...
var selRowIds = jQuery('#gridActivityTypeMaster').jqGrid('rowselect', 'selarrrow');
...
function editRow()
{
	...
	{
		jQuery("#gridActivityTypeMaster").jqGrid('editGridRow', row ,{height:230, width:425,reloadAfterSubmit:false,closeAfterEdit:true,top:0,left:350,modal:true , afterSubmit: function () {
			reloadPage();
	    }});
...
function deleteRow()
{
	  var status = jQuery("#gridActivityTypeMaster").jqGrid('getCell',row,'status');
//	  alert(status);
	if(row==0)
	{
	...
	{
		$("#gridActivityTypeMaster").jqGrid('delGridRow',row, {height:120, width:425,reloadAfterSubmit:true,closeAfterEdit:false,top:150,left:350,modal:true,caption:'Delete',msg: "Delete selected record(s)?",bSubmit: "Delete" , afterSubmit: function () {
			reloadPage();
	    }});
...
function searchRow()
{
	 $("#gridActivityTypeMaster").jqGrid('searchGrid', {sopt:['eq','cn']} );
}

function reloadPage()
{
	var grid = $("#gridActivityTypeMaster");
