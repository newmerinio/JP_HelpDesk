...
function addConfigLocation(){
	$("#data_part").html("<br><br><br><br><br><br><br><br><br><br><br><br><center><img src=images/ajax-loaderNew.gif></center>");
	$.ajax({
	    type : "post",
	    url : "view/Over2Cloud/patientCRMMaster/beforeAddConfigureLocation.action",
...
function getGridView()
{
	$.ajax({
 		type :"post",
 		url :"view/Over2Cloud/patientCRMMaster/viewConfigureLocation.action",
...
<div class="list-icon">
 ...
 <div class="clear"></div>
	...
	<sj:a id="addButton" title="Add Location" button="true"  buttonIcon="ui-icon-plus" cssClass="button" onclick="addConfigLocation();">Location </sj:a>
	<sj:a id="addBn" title="Add Territory" button="true"  buttonIcon="ui-icon-plus" cssClass="button" onclick="configureTerr();">Territory</sj:a>
    </td>   
	</tr></tbody></table>
	</div>
	</div>
...
var selRowIds = jQuery('#gridCountry').jqGrid('rowselect', 'selarrrow');
...
function editRow()
{
	...
	{
		jQuery("#gridCountry").jqGrid('editGridRow', row ,{height:230, width:425,reloadAfterSubmit:false,closeAfterEdit:true,top:0,left:350,modal:true , afterSubmit: function () {
			reloadPage();
	    }});
...
function deleteRow()
{
	  var status = jQuery("#gridCountry").jqGrid('getCell',row,'status');
//	  alert(status);
	if(row==0)
	{
	...
	{
		$("#gridCountry").jqGrid('delGridRow',row, {height:120, width:425,reloadAfterSubmit:true,closeAfterEdit:false,top:150,left:350,modal:true,caption:'Delete',msg: "Delete selected record(s)?",bSubmit: "Delete" , afterSubmit: function () {
			reloadPage();
	    }});
...
function searchRow()
{
	 $("#gridCountry").jqGrid('searchGrid', {sopt:['eq','cn']} );
}

function reloadPage()
{
	var grid = $("#gridCountry");
