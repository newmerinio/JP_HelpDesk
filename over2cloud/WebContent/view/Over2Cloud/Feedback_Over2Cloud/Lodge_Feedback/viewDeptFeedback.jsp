...
$.subscribe('getStatusFormResolved', function(event,data)
  {
	  ...
	  var resolvedBy = jQuery("#gridedittable").jqGrid('getCell',feedid,'resolve_by');
	  var resolvedOn = jQuery("#gridedittable").jqGrid('getCell',feedid,'resolve_date');
	  var resolvedAt=jQuery("#gridedittable").jqGrid('getCell',feedid,'resolve_time');
	  var resolveDuration=jQuery("#gridedittable").jqGrid('getCell',feedid,'resolve_duration');
	  ...
	  else
	  {
		  $("#data_part").html("<br><br><br><br><br><br><br><br><br><br><br><br><center><img src=images/ajax-loaderNew.gif></center>");
		    $.ajax({
		        type : "post",
		        url : connection+"/view/Over2Cloud/Feedback_Over2Cloud/Lodge_Feedback/feedActionResolved.action?feedStatus="+feedStatus.split(" ").join("%20")+"&feedId="+feedid.split(" ").join("%20")+"&ticketNo="+ticketNo.split(" ").join("%20")+"&feedbackBy="+feedby.split(" ").join("%20")+"&mobileno="+feedmob.split(" ").join("%20")+"&emailId="+feedemail.split(" ").join("%20")+"&feedBreif="+feedbrief.split(" ").join("%20")+"&openDate="+opendate.split(" ").join("%20")+"&openTime="+opentime.split(" ").join("%20")+"&subCatg="+subcatg.split(" ").join("%20")+"&allotto="+allotto.split(" ").join("%20")+"&resolvedBy="+resolvedBy.split(" ").join("%20")+"&resolvedOn="+resolvedOn.split(" ").join("%20")+"&resolvedAt="+resolvedAt.split(" ").join("%20")+"&resolveDuration="+resolveDuration.split(" ").join("%20")+"&rca="+rca.split(" ").join("%20")+"&capa="+capa.split(" ").join("%20"),
...
$.subscribe('getStatusForm', function(event,data)
  {
	  var connection=document.getElementById("conString").value;
	  var feedStatus = $("#status").val();
	  var feedid   = jQuery("#gridedittable").jqGrid('getGridParam', 'selrow');
	  var ticketNo = jQuery("#gridedittable").jqGrid('getCell',feedid,'ticket_no');
	  var feedby   = jQuery("#gridedittable").jqGrid('getCell',feedid,'feed_by');
	  var feedmob  = jQuery("#gridedittable").jqGrid('getCell',feedid,'patMobNo');
	  var feedemail= jQuery("#gridedittable").jqGrid('getCell',feedid,'patEmailId');
	  var subcatg  = jQuery("#gridedittable").jqGrid('getCell',feedid,'feedback_subcatg');
	  var feedbrief= jQuery("#gridedittable").jqGrid('getCell',feedid,'feed_brief');
	  var opendate = jQuery("#gridedittable").jqGrid('getCell',feedid,'open_date');
	  var opentime = jQuery("#gridedittable").jqGrid('getCell',feedid,'open_time');
	  var allotto = jQuery("#gridedittable").jqGrid('getCell',feedid,'feedback_allot_to');
	//  var feed_registerby=jQuery("#gridedittable").jqGrid('getCell',feedid,'feed_registerby');
	  ...
	  {
		  //alert(connection+"/view/Over2Cloud/Feedback_Over2Cloud/Lodge_Feedback/feedAction.action?feedStatus="+feedStatus.split(" ").join("%20")+"&feedId="+feedid.split(" ").join("%20")+"&ticketNo="+ticketNo.split(" ").join("%20")+"&feedbackBy="+feedby.split(" ").join("%20")+"&mobileno="+feedmob.split(" ").join("%20")+"&emailId="+feedemail.split(" ").join("%20")+"&feedBreif="+feedbrief.split(" ").join("%20")+"&openDate="+opendate.split(" ").join("%20")+"&openTime="+opentime.split(" ").join("%20")+"&subCatg="+subcatg.split(" ").join("%20")+"&allotto="+allotto.split(" ").join("%20")+"&feed_registerby="+feed_registerby.split(" ").join("%20"));
	//	  alert(feedStatus+">>"+feedid+">>"+ticketNo+">>"+feedby+">>"+feedemail+">>"+feedmob+">>"+subcatg+">>"+feedbrief+">>"+opendate+">>"+opentime+">>"+allotto+">>");
		  $("#data_part").html("<br><br><br><br><br><br><br><br><br><br><br><br><center><img src=images/ajax-loaderNew.gif></center>");
		    $.ajax({
		        type : "post",
...
function reloadResolved(rowid, result)
{
	$.ajax({
	    type : "post",
	    url : "/over2cloud/view/Over2Cloud/Feedback_Over2Cloud/Lodge_Feedback/beforeFeedActionDept.action?feedStatus=Resolved",
	    success : function(subdeptdata) {
$("#"+"data_part").html(subdeptdata);
	},
	   error: function() {
...
function getSearchData()
{
	...
	$.ajax({
 		type :"post",
 		url : "view/Over2Cloud/Feedback_Over2Cloud/Lodge_Feedback/viewSearchedFeedForAction.action?feedStatus="+status.split(" ").join("%20")+"&fromDate="+fromDate+"&toDate="+toDate+"&mode="+mode.split(" ").join("%20")+"&feedType="+feedType.split(" ").join("%20")+"&feedCat="+feedCat.split(" ").join("%20")+"&ticket_no="+ticket_no.split(" ").join("%20")+"&feedBy="+feedBy.split(" ").join("%20")+"&crNo="+crNo.split(" ").join("%20"),
 		success : function(data) 
	    {
			$("#feedTicket").html(data);
		},
...
<div class="list-icon">
	...
	<div class="head"><img alt="" src="images/forward.png" style="margin-top:50%; float: left;"></div><div class="head">Total: <s:property value="%{totalTickets}"/><sj:a id="refButtonPage1" title="Refresh" buttonIcon="ui-icon-refresh" cssStyle="height:25px; width:32px;margin-left: 742px;" cssClass="button" button="true" onclick="reloadPage();"></sj:a></div>
</div>
<div class="clear"></div>
		...
		<tbody>
			<tr>
				  ...
				  <td>
					  <table class="floatL" border="0" cellpadding="0" cellspacing="0">
					    <tbody><tr><td class="pL10">
					        ...
					        <s:select 
		                      id="status" 
					          name="status" 
					          list="{'High Priority','Ignore','Noted','Pending','Resolved','Snooze'}"
					          headerKey="Pending"
					          headerValue="Status"
					          cssClass="button"
					          cssStyle = "margin-left: 0px;margin-top:-31px;height:27px;width:90px"
					          theme="simple"
			          ...
			          headerValue="Mode"
			          cssClass="button"
			          cssStyle = "margin-left: 0px;margin-top:-31px;height:27px"
			          theme="simple"
			          onchange="getSearchData();"
         			 />
         			 <s:select 
			          ...
			          cssClass="button"
			          cssStyle = "width: 105px;margin-left: 3px;margin-top:-31px;height:27px"
			          theme="simple"
			          onchange="getSearchData();"
         			 />
         			 <s:select 
			          ...
			          onchange="getSearchData();"
         			 />
         			 <s:textfield theme="simple" id="crNo" placeholder="Enter CR No" cssClass="button" cssStyle = "height:15px;margin-top:-26px;margin-left: 3px" onkeyup="getSearchData();"/>
					         </td></tr></tbody>
					  </table>
				  </td>
				  ...
				  <div id="takeAction">
				  	<%if(userRights.contains("doti") )
				      {
					%><sj:submit cssStyle="height:27px;" value="Action" button="true" onClickTopics="getStatusForm" cssClass="button"/>
					<%	}%>
				  </div>
				  <div id="addComments" style="display: none">
				  	<%if(userRights.contains("doti") )
				      {
					%><sj:submit cssStyle="height:27px;" value="Update Action" button="true" onClickTopics="getStatusFormResolved" cssClass="button"/>
					<%	}%>
				  </div>
