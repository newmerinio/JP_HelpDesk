...
function addPackage(){
	...
	$.ajax({
	    type : "post",
	    url : "view/Over2Cloud/CorporatePatientServices/cpservices/beforeAddPackage.action",
...
function getGridView()
{
	$.ajax({
 		type :"post",
 		url :"view/Over2Cloud/CorporatePatientServices/cpservices/viewPackageDetails.action",
...
<div class="list-icon">
 ...
 <div class="clear"></div>
	...
	<sj:a id="addButton"  button="true"  buttonIcon="ui-icon-plus" cssClass="button" onclick="addPackage();">Add</sj:a>
    </td>   
	</tr></tbody></table>
	</div>
	</div>
...
var selRowIds = jQuery('#gridPackage').jqGrid('rowselect', 'selarrrow');
...
function editRow()
{
	...
	{
		jQuery("#gridPackage").jqGrid('editGridRow', row ,{height:230, width:425,reloadAfterSubmit:false,closeAfterEdit:true,top:0,left:350,modal:true , afterSubmit: function () {
			reloadPage();
	    }});
...
function deleteRow()
{
	  var status = jQuery("#gridPackage").jqGrid('getCell',row,'status');
//	  alert(status);
	if(row==0)
	{
	...
	{
		$("#gridPackage").jqGrid('delGridRow',row, {height:120, width:425,reloadAfterSubmit:true,closeAfterEdit:false,top:150,left:350,modal:true,caption:'Delete',msg: "Delete selected record(s)?",bSubmit: "Delete" , afterSubmit: function () {
			reloadPage();
	    }});
...
function searchRow()
{
	 $("#gridPackage").jqGrid('searchGrid', {sopt:['eq','cn']} );
}
