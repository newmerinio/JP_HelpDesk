...
<script type="text/javascript">
	...
	function getEntryFields(vehiclestatus) {
	    var vehicleExitStatus = $('#vehicleExitStatus').val();
		$.ajax({
			type : "post",
		    url : "view/Over2Cloud/VAM/master/vehicleExitAdd.action?modifyFlag=0&deleteFlag=0"+"&vehiclestatus="+vehiclestatus+"&vehicleExitStatus="+vehicleExitStatus,
		    success : function(data) {
	     	  $("#"+"data_part").html(data);
...
<body><!--
	...
	<s:form id="formone" name="formone" namespace="/view/Over2Cloud/VAM/master" action="vehicleExitSubmit" theme="css_xhtml"  method="post" >
	        ...
	        <s:hidden name="sr_number"  id="sr_number"/>
	  ...
	  <s:iterator value="vehiclefields" status="counter">
	                <s:if test="%{key != 'sr_number'}">
					    ...
					    <s:hidden id="%{key}" name="%{key}"></s:hidden>	
						</s:elseif>
     ...
     --><s:if test="%{locationNameExit != null}">
					...
					<s:if test="%{mandatory}">
						...
						<s:if test="%{vehicleEntrydateExist != null}">
         				...
         				<!--<s:hidden name="sr_number"  id="sr_number"   cssClass="textField" readonly="true" placeholder="Enter Data" cssStyle="margin:0px 0px 10px 0px"/>
