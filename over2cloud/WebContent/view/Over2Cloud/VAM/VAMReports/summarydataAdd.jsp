...
<script type="text/javascript">
	...
	<script type="text/javascript">
			$.subscribe('presult', function(event,data)
		       {
		         setTimeout(function(){ $("#summaryrepaddition").fadeIn(); }, 10);
		         setTimeout(function(){ $("#summaryrepadditionss").fadeOut(); }, 4000);
		       });
	</script>
	<script type="text/javascript">
...
<body>
<div class="list-icon">
	<div class="head">Summary Report</div><div class="head"><img alt="" src="images/forward.png" style="margin-top:50%; float: left;"></div><div class="head">Add</div>
	</div>
	<div class="clear"></div>
	...
	<s:form id="formone" name="formone" namespace="/view/Over2Cloud/VAM/reports" action="summarydataSubmit" theme="css_xhtml"  method="post" >
	...
	<!-- Drop down -->
		<s:iterator value="deptDropDown" status="counter">
				...
				<span id="mandatoryFields" class="dIds" style="display: none; "><s:property value="%{key}"/>#<s:property value="%{value}"/>#<s:property value="%{colType}"/>#<s:property value="%{validationType}"/>,</span>
					...
					<s:if test="%{locationNameExit != null}">
         			...
         			<s:if test="%{gateNameExist != null}">
						<div class="newColumn">
						<div class="leftColumn1"><s:property value="%{gatename}"/>:</div>
						<div class="rightColumn1">
         			...
         			<s:if test="%{visitedDeptExist != null}">
						<div class="newColumn">
						<div class="leftColumn1"><s:property value="%{visiteddept}"/>:</div>
						<div class="rightColumn1">
						<s:if test="%{visitedDeptExist == 'true'}"><span class="needed"></span></s:if>
						         <s:select 
						                     id="dept"
						                     name="dept" 
						                     list="departmentlist"
         			...
         			<s:iterator value="summaryrepfields" status="counter">
						<s:if test="%{mandatory}">
							<span id="mandatoryFields" class="dIds" style="display: none; "><s:property value="%{key}" />#<s:property value="%{value}" />#<s:property value="%{colType}" />#<s:property value="%{validationType}" />,</span>
						</s:if>
							<div class="newColumn">
								...
								<div class="rightColumn1">
									<sj:datepicker id="%{key}" name="%{key}" value="%{new java.util.Date()}" readonly="true" showOn="focus" cssClass="textField" timepicker="true" timepickerOnly="true"/>
								</div>
							</div>
					</s:iterator>
					<s:if test="%{reportforExist != null}">
						<div class="newColumn">
						<div class="leftColumn1"><s:property value="%{reportfor}"/>:</div>
						<div class="rightColumn1">
						<s:if test="mandatory">
							<span class="needed"></span>
						</s:if>
						<s:if test="%{reportforExist == 'true'}"><span class="needed"></span></s:if>
						         <s:select 
						                     id="report_for"
						                     name="report_for" 
						                     list="#{'Visitor':'Visitor', 'Vehicle':'Vehicle', 'Post':'Post'}"
						                     headerKey="-1"
						                     headerValue="All" 
						                     cssClass="select"
						                     cssStyle="width:82%"
	        ...
	        <sj:a 
						...
						indicator="indicator" 
						button="true" 
						id="summaryrepaddCancel"
						>
						Back
		  </sj:a>
			</div>
			</div>
					<sj:div id="summaryrepaddition"  effect="fold">
					 <div id="summaryrepaddresult"></div>
					</sj:div>
			</div>
	   		</center>
