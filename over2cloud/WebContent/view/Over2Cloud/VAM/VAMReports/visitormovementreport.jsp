<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	...
	function editRow()
	{
		...
		jQuery("#vistormovementviewid").jqGrid('editGridRow', row ,{width:425,reloadAfterSubmit:true,closeAfterEdit:true,top:0,left:350,modal:true});
	}
	...
	function deleteRow()
	{
		...
		$("#vistormovementviewid").jqGrid('delGridRow',row, {height:120,reloadAfterSubmit:false,top:0,left:350,modal:true});
	}
	function searchRow()
	{
		jQuery("#vistormovementviewid").jqGrid('searchGrid', {multipleSearch:false,reloadAfterSubmit:true,top:0,left:350,modal:true} );
	}
	function refreshRow(row, result) 
	{
		  $("#vistormovementviewid").trigger("reloadGrid"); 
	}
</script>
<SCRIPT type="text/javascript">
	 ...
	 function formatImage(cellvalue, options, row) {
		 ...
		 cellvalue="Search";
	 	return "<img title='Mail Visitor'  src='"+ context_path +"/images/mail-message-new.png' onClick='pageisNotAvailablelll();' />" 
		;}
...
<script type="text/javascript">
	...
	<body>
	<div class="list-icon">
		<s:if test="%{visitor_status == 'visitorstatus'}">
	<div class="head">Visitor Status</div><div class="head"><img alt="" src="images/forward.png" style="margin-top:50%; float: left;"></div><div class="head">View</div>
	</s:if>
	<s:else>
	...
	<div class="listviewBorder">
			...
			<span class="normalDropDown">
					     ...
					     <sj:a button="true" cssClass="button" cssStyle="height:25px;" title="Visitor In" buttonIcon="" id="stillInVisitorId">Visitor In</sj:a>
						 <sj:a button="true" cssClass="button" cssStyle="height:25px;" title="Visitor Out" buttonIcon="" id="outVisitorId">Visitor Out</sj:a>
						<s:if test="%{visitor_status!='visitorstatus'}">
						<sj:a button="true" cssClass="button" cssStyle="height:25px;" title="Back" buttonIcon="" id="cancelVisitorMovementId">Back</sj:a>
						</s:if>
		...
		<s:url id="visitormovementRecord" action="visitormovementView">
		</s:url>
		<s:url id="modifyVisitorMovementGrid" action="modifyVisitorMovementGrid" />
		<sjg:grid 
			  id="vistormovementviewid"
	          href="%{visitormovementRecord}"
	          gridModel="visitorMovementData"
	          navigator="true"
	          navigatorAdd="false"
	          navigatorView="true"
	          ...
	          editurl="%{modifyVisitorMovementGrid}"
		...
		<s:iterator value="viewVisitorMovementDetail" id="viewVisitorMovementDetail" >  
		<sjg:gridColumn 
			name="%{colomnName}"
			index="%{colomnName}"
			title="%{headingName}"
