<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	...
	function editRow()
	{
		...
		jQuery("#scheduledrequestViewid").jqGrid('editGridRow', row ,{width:425,reloadAfterSubmit:true,closeAfterEdit:true,top:0,left:350,modal:true});
	}
	...
	function deleteRow()
	{
		...
		$("#scheduledrequestViewid").jqGrid('delGridRow',row, {height:120,reloadAfterSubmit:false,top:0,left:350,modal:true});
	}
	function searchRow()
	{
		jQuery("#scheduledrequestViewid").jqGrid('searchGrid', {multipleSearch:false,reloadAfterSubmit:true,top:0,left:350,modal:true} );
	}
	function refreshRow(row, result) 
	{
		  $("#scheduledrequestViewid").trigger("reloadGrid"); 
	}
</script>
<SCRIPT type="text/javascript">
	 ...
	 function formatImage(cellvalue, options, row) {
		...
		"&nbsp&nbsp<img title='Mail Scheduled Visitor'  src='"+ context_path +"/images/mail-message-new.png' onClick='pageisNotAvailable();' />"
	 	;}
...
function sendmaildownloadpdf(xxxx){
	...
	var subject= "Invoice "+xxxx+"  from "+s+" is attached with PDF";
	...
	<div class="listviewBorder">
			...
			<span class="normalDropDown">
					     ...
					     <sj:a button="true" cssClass="button" cssStyle="height:25px;" title="Back" buttonIcon="" id="cancelscheduledvisitorview">Back</sj:a>
			</td>   
			</tr></tbody></table></div>	</div>
			<div class="clear"></div>
		<s:url id="scheduledprerequestview" action="scheduledprerequestview"></s:url>
		<s:url id="modifyScheduledVisitorGrid" action="modifyScheduledVisitorGrid" />
		<div style="overflow: scroll; height: 480px;">
		<sjg:grid 
			  id="scheduledrequestViewid"
	          href="%{scheduledprerequestview}"
	          gridModel="scheduledvisitorData"
	          navigator="true"
	          navigatorAdd="false"
	          navigatorView="true"
	          ...
	          editurl="%{modifyScheduledVisitorGrid}"
		...
		<s:iterator value="viewScheduledrequestDetail" id="viewScheduledrequestDetail" >  
		<sjg:gridColumn 
			name="%{colomnName}"
			index="%{colomnName}"
			title="%{headingName}"
