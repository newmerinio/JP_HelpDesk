	...
	$.subscribe('presult', function(event,data)
		       {
		         setTimeout(function(){ $("#venderaddition").fadeIn(); }, 10);
		         setTimeout(function(){ $("#venderaddition").fadeOut(); }, 4000);
		       });
...
function loadVendorCompName(){
	var id = $("#vendor_type option:selected").text();
	if(id == null || id == 'Select Vendor Type')
	return;
	$.ajax({
	type : "post",
	url : "view/Over2Cloud/VAM/master/loadvendorcompnamefront.action",
	data: "id="+id, 
	success : function(data) {
		$('#brief option').remove();
		$('#brief').append($("<option></option>").attr("value",-1).text('Select'));
		$(data).each(function(index)
		{
		   $('#brief').append($("<option></option>").attr("value",this.NAME).text(this.NAME));
		});
	},
	error: function() {
		alert("error");
	...
	<body>
	<div class="list-icon">
	<div class="head">Vendor</div><div class="head"><img alt="" src="images/forward.png" style="margin-top:50%; float: left;"></div><div class="head">Register</div>
	</div>
	<div class="clear"></div>
	...
	<s:form id="formone" name="formone" namespace="/view/Over2Cloud/VAM/master" action="venderSubmit" theme="css_xhtml"  method="post" >
	...
	<div class="clear"></div>
					<s:iterator value="vendordropdownlist" status="counter">
					<s:if test="%{mandatory}">
		              <span id="mandatoryFields" class="dIds" style="display: none;"><s:property value="%{key}" />#<s:property value="vendor_type" />#<s:property value="%{colType}" />#<s:property value="%{validationType}" />,</span>
		            </s:if>
					</s:iterator>
					<s:if test="%{vendorTypeExist != null}">
         			...
         			<s:if test="%{vendorcompNameExist != null}">
						<div class="newColumn">
						<div class="leftColumn1"><s:property value="%{vendorCompName}"/>:</div>
						<div class="rightColumn1">
						<s:if test="%{vendorcompNameExist == 'true'}"><span class="needed"></span></s:if>
						         <s:select 
						                     id="brief"
						                     name="brief" 
						                     list="vendorcompFrontlist"
						                     headerKey="-1"
	        ...
	        <sj:submit 
	         ...
	         <sj:a 
	          ...
	          <sj:a 
			   		...
			   		<sj:div id="venderaddition"  effect="fold">
					 <div id="venderresult"></div>
					</sj:div>
	   		</center>
