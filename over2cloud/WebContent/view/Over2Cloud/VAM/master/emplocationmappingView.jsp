...
<title>Employee Location Mapping</title>
<script type="text/javascript">
	...
	function editRow()
	{
		...
		jQuery("#emplocmapidid").jqGrid('editGridRow', row ,{width:425,reloadAfterSubmit:true,closeAfterEdit:true,top:0,left:350,modal:true});
	}
	...
	function deleteRow()
	{
		...
		$("#emplocmapidid").jqGrid('delGridRow',row, {height:120,reloadAfterSubmit:false,top:0,left:350,modal:true});
	}
	function searchRow()
	{
		jQuery("#emplocmapidid").jqGrid('searchGrid', {multipleSearch:false,reloadAfterSubmit:true,top:0,left:350,modal:true} );
	}
	function refreshRow(row, result) 
	{
		  $("#emplocmapidid").trigger("reloadGrid"); 
	}
</script>
...
<body>
		...
		<div class="listviewButtonLayer" id="listviewButtonLayerDiv" style="height: 40px;">
			<div class="pwie">
			 ...
			 <span class="normalDropDown">
				...
				<sj:a button="true" cssClass="button" cssStyle="height:25px;" title="Add" buttonIcon="ui-icon-plus" id="addemplocgatemap">Add</sj:a>
			</td>   
			</tr></tbody></table></div>	</div>
			<div class="clear"></div>
		<s:url var="app_name" id="getemplocmappingDetails" action="getemplocmappingDetails" escapeAmp="false"> </s:url>
		<s:url id="modifyemplocmapGrid" action="modifyemplocmapGrid" />
		<div style="overflow: scroll; height: 480px;">
		<sjg:grid 
			  id="emplocmapidid"
	          href="%{getemplocmappingDetails}"
	          gridModel="emplocgatemapdata"
	          groupField="['deptName']"
	          groupCollapse="true"
		...
		<s:iterator value="viewEmplocmappingDetail" id="viewEmplocmappingDetail" >  
		<sjg:gridColumn 
			name="%{colomnName}"
			index="%{colomnName}"
			title="%{headingName}"
