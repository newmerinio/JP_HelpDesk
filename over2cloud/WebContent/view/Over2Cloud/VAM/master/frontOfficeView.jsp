	...
	function editRow()
	{
		...
		jQuery("#frontoffcViewid").jqGrid('editGridRow', row ,{width:425,reloadAfterSubmit:true,closeAfterEdit:true,top:0,left:350,modal:true});
	}
	...
	function deleteRow()
	{
		...
		$("#frontoffcViewid").jqGrid('delGridRow',row, {height:120,reloadAfterSubmit:false,top:0,left:350,modal:true});
	}
	function searchRow()
	{
		jQuery("#frontoffcViewid").jqGrid('searchGrid', {multipleSearch:false,reloadAfterSubmit:true,top:0,left:350,modal:true} );
	}
	function refreshRow(row, result) 
	{
		  $("#frontoffcViewid").trigger("reloadGrid"); 
	}
</script>
	</head>
	<body>
	...
	<div class="listviewBorder">
			...
			<span class="normalDropDown">
					     ...
					     <sj:a button="true" cssClass="button" cssStyle="height:25px;" title="Add" buttonIcon="ui-icon-plus" id="addVenderId">Add</sj:a>
			</td>   
			</tr></tbody></table></div>	</div>
			<div class="clear"></div>
		<s:url id="frontoffcRecord" action="frontoffcDetails"></s:url>
		<s:url id="modifyfrontOffcGrid" action="modifyfrontOffcGrid" />
		<div style="overflow: scroll; height: 480px;">
		<sjg:grid 
			  id="frontoffcViewid"
	          href="%{frontoffcRecord}"
	          gridModel="vender"
	          navigator="true"
	          navigatorAdd="false"
	          navigatorView="true"
		...
		<s:iterator value="viewFrontoffcDetail" id="viewFrontoffcDetail" >  
		<sjg:gridColumn 
			name="%{colomnName}"
			index="%{colomnName}"
			title="%{headingName}"
