...
<script type="text/javascript">
	...
	function editRow()
	{
		...
		jQuery("#purposeView").jqGrid('editGridRow', row ,{width:425,reloadAfterSubmit:true,closeAfterEdit:true,top:0,left:350,modal:true});
	}
	...
	function deleteRow()
	{
		...
		$("#purposeView").jqGrid('delGridRow',row, {height:120,reloadAfterSubmit:false,top:0,left:350,modal:true});
	}
	function searchRow()
	{
		jQuery("#purposeView").jqGrid('searchGrid', {multipleSearch:false,reloadAfterSubmit:true,top:0,left:350,modal:true} );
	}
	function refreshRow(row, result) 
	{
		  $("#purposeView").trigger("reloadGrid"); 
	}
</script>
</head>
<body>
...
<div class="listviewBorder">
		...
		<span class="normalDropDown">
					     ...
					     <sj:a button="true" cssClass="button" cssStyle="height:25px;" title="Add" buttonIcon="ui-icon-plus" id="addPurposeId">Add</sj:a>
		</td>   
		</tr></tbody></table></div>	</div>
		<div class="clear"></div>
	<s:url id="purposeRecord" action="purposeRecords"></s:url>
	<s:url id="modifyPurposeGrid" action="modifyPurposeGrid" />
	<div style="overflow: scroll; height: 480px;">
	<sjg:grid 
		  id="purposeView"
          href="%{purposeRecord}"
          gridModel="purposeList"
          groupField="['purpose']"
	      groupCollapse="true"
	      groupColumnShow="[false]"
	      groupText="['<b>{0} - {1} Alert After</b>']"
          navigator="true"
          navigatorAdd="false"
          navigatorView="true"
	...
	<s:iterator value="viewPurpose" id="viewPurpose" >  
	<sjg:gridColumn 
		name="%{colomnName}"
		index="%{colomnName}"
		title="%{headingName}"
