	   ...
	   if($("#"+fieldsvalues).val().length > 10 || $("#"+fieldsvalues).val().length < 10)
		...
		break;
      	  ...
      	  if($("#"+fieldsvalues).val().length < 1)
	             {
	                rserrZone.innerHTML="<div class='user_form_inputError2'>Enter "+fieldsnames+" !!! </div>";
	                $("#"+fieldsvalues).css("background-color","#ff701a");  //255;165;79
	                $("#"+fieldsvalues).focus();
	                setTimeout(function(){ $("#rserrZone").fadeIn(); }, 10);
	                setTimeout(function(){ $("#rserrZone").fadeOut(); }, 2000);
...
for(var i=0; i<fieldtype.length; i++)
{
	...
	if(fieldsvalues!= "" )
	{
	   ...
	   if($("#"+fieldsvalues).val().length > 10 || $("#"+fieldsvalues).val().length < 10)
	    ...
	    else if (!pattern.test($("#"+fieldsvalues).val().trim().length < 1)) {
...
$.subscribe('validateUpload', function(event,data)
		{
	
			var mystring=null;
			mystring=$(".pIdsKR").text();
			var fieldtype = mystring.split(",");
		    var pattern = /^\d{10}$/;
		    for(var i=0; i<fieldtype.length; i++)
		    {
