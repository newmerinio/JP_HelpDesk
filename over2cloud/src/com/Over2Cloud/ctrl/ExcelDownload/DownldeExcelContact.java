...
public class DownldeExcelContact {
	/**
	 *  Define Constructure
	 */
	public DownldeExcelContact() {
	}
	...
	public String createExcel(String titleString, String sheetString,String subTitleString, List data, String[] headerTitles,String titleKey, String filePath){
		...
		try{
    		...
    		List<ConfigurationUtilBean> titleMap = new ContactCommonPropertyMap().getTitles(titleKey);
			...
			if (headerTitles != null ){
				for (ConfigurationUtilBean cell : titleMap) {
					for (int i = 0; i < headerTitles.length; i++) {
						if(cell.getKey().equalsIgnoreCase(headerTitles[i].trim())){
	...
	public String createPdf(List data,String[] headerTitles,String titleKey, String filePath)
	{
		...
		try {
			...
			fileName = filePath + File.separator + "Contact Report"
			+ DateUtil.getCurrentDateIndianFormat()
		...
		return fileName;
}	// method to download statement Report.
	@SuppressWarnings("unchecked")
	public String createstatementReportPdf(List data,String[] headerTitles, String filePath, String customerName,String CustomerType, String GrandTotal, String PaidAmount, String owing)
	{
		int col = 0;
		String fileName=null;
		PdfPCell cell = null;
		try {
			...
			Document document=new Document(PageSize.A4,20,20,50,50);
			fileName = filePath + File.separator + "Statment Report"
			+ DateUtil.getCurrentDateIndianFormat()
			+ (DateUtil.getCurrentTime()).replaceAll(":", "-") + ".pdf";
			 PdfWriter.getInstance(document,new FileOutputStream(fileName));
			 document.open();
			 PdfPTable table=new PdfPTable(col);
			 //before header data
			 PdfPTable tables = new PdfPTable(2); // 3 columns.
			 	tables.setWidthPercentage(100f);
	            PdfPCell cell1 = new PdfPCell(new Paragraph("Customer Type"));
	            cell1.setBackgroundColor(new BaseColor(204,204,204));
	            PdfPCell cell2 = new PdfPCell(new Paragraph(CustomerType));
	            cell2.setBackgroundColor(new BaseColor(204,204,204));
	            PdfPCell cell3 = new PdfPCell(new Paragraph("Customer Name"));
	            PdfPCell cell4 = new PdfPCell(new Paragraph(customerName));
	            PdfPCell cell5 = new PdfPCell(new Paragraph("Grand Total"));
	            cell5.setBackgroundColor(new BaseColor(204,204,204));
	            PdfPCell cell6 = new PdfPCell(new Paragraph(GrandTotal));
	            cell6.setBackgroundColor(new BaseColor(204,204,204));
	            PdfPCell cell7 = new PdfPCell(new Paragraph("Paid"));
	            PdfPCell cell8 = new PdfPCell(new Paragraph(PaidAmount));
	            PdfPCell cell9 = new PdfPCell(new Paragraph("Owing"));
	            PdfPCell cell10 = new PdfPCell(new Paragraph(owing));
			 ...
			 if (headerTitles != null ){
					 for (int i = 0; i < headerTitles.length; i++) {
						cell=new PdfPCell(new Paragraph(headerTitles[i]));
						cell.setBackgroundColor(new BaseColor(204,204,204));
						cell.setBorderColor(new BaseColor(170,170,170));
						cell.setHorizontalAlignment(Element.ALIGN_CENTER);
						cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
	...
	//method to create printPreview PDF
	@SuppressWarnings("unchecked")
	public String createstatementReportPdf(String[] data,String[] headerTitles, String filePath)
	{
		int col = 0;
		String fileName=null;
		PdfPCell cell = null;
		try {
			...
			Document document=new Document(PageSize.A4,20,20,50,50);
			fileName = filePath + File.separator + "Print Preview Report"
			+ DateUtil.getCurrentDateIndianFormat()
			+ (DateUtil.getCurrentTime()).replaceAll(":", "-") + ".pdf";
			 PdfWriter.getInstance(document,new FileOutputStream(fileName));
			 document.open();
			    ...
			    String cellValue="NA";
			     try{
		  	     if(data!=null && data.length>0){
							for (int cellIndex = 0; cellIndex < headerTitles.length; cellIndex++) {
								if(data[cellIndex]!=null && !data[cellIndex].toString().equalsIgnoreCase("")){
									cell=new PdfPCell(new Paragraph(data[cellIndex].toString()));}
								 else
								  cell=new PdfPCell(new Paragraph(cellValue));
	...
	// method to create statement print Preview.
	@SuppressWarnings("unchecked")
	public String createstatementPrintPdf(List data,String[] headerTitles, String filePath,String CustomerType, String customerName, String GrandTotal, String PaidAmount, String owing)
	{
		int col = 0;
		String fileName=null;
		PdfPCell cell = null;
		try {
			...
			Document document=new Document(PageSize.A4,20,20,50,50);
			fileName = filePath + File.separator + "Statement PrintPreview Report"
			+ DateUtil.getCurrentDateIndianFormat()
			+ (DateUtil.getCurrentTime()).replaceAll(":", "-") + ".pdf";
			 PdfWriter.getInstance(document,new FileOutputStream(fileName));
			 document.open();
			 ...
			 PdfPTable tables = new PdfPTable(2); // 3 columns.
	            ...
	            PdfPCell cell1 = new PdfPCell(new Paragraph("Customer Type"));
	            cell1.setBackgroundColor(new BaseColor(204,204,204));
	            String[] custType=CustomerType.split(",");
	            PdfPCell cell2 = new PdfPCell(new Paragraph(custType[0]));
	            cell2.setBackgroundColor(new BaseColor(204,204,204));
	            String[] custName=customerName.split(",");
	            PdfPCell cell3 = new PdfPCell(new Paragraph("Customer Name"));
	            PdfPCell cell4 = new PdfPCell(new Paragraph(custName[0]));
	            PdfPCell cell5 = new PdfPCell(new Paragraph("Grand Total"));
	            cell5.setBackgroundColor(new BaseColor(204,204,204));
	            String[] Total=GrandTotal.split(",");
	            PdfPCell cell6 = new PdfPCell(new Paragraph(Total[0]));
	            cell6.setBackgroundColor(new BaseColor(204,204,204));
	            String[] Paid=PaidAmount.split(",");
	            PdfPCell cell7 = new PdfPCell(new Paragraph("Paid"));
	            PdfPCell cell8 = new PdfPCell(new Paragraph(Paid[0]));
	            String[] due=owing.split(",");
	            due[0]=new Integer(Integer.parseInt(Total[0])-Integer.parseInt(Paid[0])).toString();
	            PdfPCell cell9 = new PdfPCell(new Paragraph("Owing"));
	            PdfPCell cell10 = new PdfPCell(new Paragraph(due[0]));
	            tables.addCell(cell1);
	            tables.addCell(cell2);
	            tables.addCell(cell3);
	            tables.addCell(cell4);
