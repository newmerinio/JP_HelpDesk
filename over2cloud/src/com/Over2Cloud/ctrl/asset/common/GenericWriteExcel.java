...
public class GenericWriteExcel
{
	...
	public String createExcel(String titleString, String subTitleString, List dataList, List headerTitle, String filePath, String fileName)
	{
		...
		try
		{
			...
			HSSFSheet sheet = wb.createSheet(subTitleString + " On " + DateUtil.getCurrentDateIndianFormat());
				...
				for (Iterator iterator = dataList.iterator(); iterator.hasNext();)
				{
					...
					for (int cellIndex = 0; cellIndex < headerTitle.size(); cellIndex++)
					{
						...
						if (object[cellIndex] != null)
						{
							...
							{
								//dataCell.setCellValue(DateUtil.makeTitle(object
								// [cellIndex].toString()));
			...
			/*
			 * response.setHeader("Content-Disposition","attachment; filename="+subTitleString
			 * +".xls"); ServletOutputStream out = response.getOutputStream();
		...
		{
			log.error("Date : " + DateUtil.getCurrentDateIndianFormat() + " Time: " + DateUtil.getCurrentTime() + " " + "Problem in Over2Cloud  method createExcel of class " + getClass(), e);
			e.printStackTrace();
		}
		return file;
