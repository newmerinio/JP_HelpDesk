...
public class GroupdataAdd extends ActionSupport{
	...
	public String editgroupaction(){
		String errorString=ERROR;
		Object[] object=null;
		StringBuilder stringobj=new StringBuilder();
		try{
		   System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>"+getId());
			if(getOper()!=null && getOper()!=""){
				String[] arrayId=getId().split("-");
				CommonforClassdata obhj = new CommonOperAtion();
				Map<String,Object> whereClose=new HashMap<String, Object>();
				...
				listdata.add("contactid");
				listdata.add("groupid");
				whereClose.put("groupid", arrayId[1]);
				List datatemp= obhj.getSelectdataFromSelectedFields("croupdata",listdata, whereClose, connectionSpace);
				System.out.println("*******************"+datatemp.size());
				for (Iterator iterator = datatemp.iterator(); iterator.hasNext();) {
				  object = (Object[]) iterator.next();
				}
				String[] contactid=object[1].toString().split(",");
				int j=1;
				for (int i = 0; i < contactid.length; i++) {
					if(contactid[i].contains(arrayId[0])){
					}else{
			            if(j<contactid.length){
						stringobj.append(contactid[i]);
						stringobj.append(",");
			            }else{
			            	stringobj.append(contactid[i]);
			            }
				...
				SetMap.put("contactid", stringobj.toString());
				boolean status=obhj.updateIntoTable("croupdata", SetMap, whereClose, connectionSpace);
				if(status){
					addActionMessage("Data Update Sucessfully.");
				}else{
					addActionError("Opss There is Prblem.");
				}
			}
			return errorString ;
		} catch(Exception e){
